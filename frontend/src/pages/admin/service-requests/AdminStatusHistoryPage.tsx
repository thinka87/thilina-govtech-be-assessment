...
const AdminStatusHistoryPage: React.FC = () => {
      ...
      {loading ? (
        ...
        <Card>
          <div className="text-center py-10">
            <p className="text-sm font-medium text-gray-600">No status history available for this request.</p>
            <p className="text-xs text-gray-400 mt-1">Status transitions will appear here as the request is processed.</p>
          </div>
        </Card>
      ) : (
        <Card padding="none">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0 pb-2">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      {entry.oldStatus ? (
                        <>
                          <Badge label={entry.oldStatus} variant={entry.oldStatus as ServiceRequestStatus} />
                          <span className="text-gray-400 text-sm">→</span>
                        </>
                      ) : (
                        <span className="text-xs text-gray-400 italic mr-1">Initial submission</span>
                      )}
                      <Badge label={entry.newStatus} variant={entry.newStatus as ServiceRequestStatus} />
                    </div>
...
export default AdminStatusHistoryPage;
