...
const AgentRequestListPage: React.FC = () => {
  ...
  const [committedCitizenRef, setCommittedCitizenRef]   = useState('');
  const [serviceTypeInput, setServiceTypeInput]         = useState('');
  const [committedServiceType, setCommittedServiceType] = useState('');
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      const data = await serviceRequestApi.searchServiceRequests({
        page,
        size: PAGE_SIZE,
        ...(committedCitizenRef  ? { citizenReference: committedCitizenRef } : {}),
        ...(committedServiceType ? { serviceType: committedServiceType }      : {}),
  ...
  }, [page, committedCitizenRef, committedServiceType, statusFilter]);

  useEffect(() => { fetchRequests(); }, [fetchRequests]);

  const handleSearch = () => {
    setPage(0);
    setCommittedCitizenRef(citizenRefInput.trim());
    setCommittedServiceType(serviceTypeInput.trim().replace(/\s+/g, '_'));
  };

  const handleClear = () => {
    setCitizenRefInput('');
    setCommittedCitizenRef('');
    setServiceTypeInput('');
    setCommittedServiceType('');
    setStatusFilter('');
    setPage(0);
  };
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Review and process citizen service requests."
      />
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-xs text-gray-400 mt-1">
              {committedCitizenRef || committedServiceType || statusFilter
                  ...
                  <tr key={req.requestReference} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => navigate(`/agent/requests/${req.requestReference}`)}
                      >
                        View
                      </Button>
                    </td>
